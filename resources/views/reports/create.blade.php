...
<div class="form-shell">
    ...
    <div class="form-layout">
        <section class="form-intro">
            ...
            <p>Setiap laporan akan diterima Guru BK dan ditangani dengan prinsip kerahasiaan serta perlindungan peserta didik.</p>
        ...
        <form class="report-form" method="POST" action="{{ route('reports.store') }}">
            ...
            <div class="two-fields">
                <label>Peran pelapor <span>*</span>
                    <select name="reporter_role" required>
                        ...
                        <option value="CONCERNED_PERSON" @selected(old('reporter_role') === 'CONCERNED_PERSON')>Saya mengetahui kejadian</option>
                    </select>
                </label>
            ...
            <div class="two-fields">
                <label>Nama pelapor <span id="reporter-name-asterisk">*</span>
                    <input type="text" name="reporter_name" id="reporter-name-input" value="{{ old('reporter_name') }}" placeholder="Nama lengkap Anda">
                    <small id="reporter-name-help">Wajib diisi untuk mode Rahasia atau Terbuka.</small>
                </label>
                <label>Kontak
                    <input type="text" name="reporter_contact" value="{{ old('reporter_contact') }}" placeholder="No. HP / WhatsApp (opsional)">
                </label>
            </div>
                ...
                <label>Kategori kejadian <span>*</span>
                    <select name="category" id="category-select" required>
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->name }}" data-category-id="{{ $category->id }}" @selected(old('category') === $category->name)>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </label>
                <label>Subkategori
                    <select name="subcategory" id="subcategory-select">
                        ...
                        @foreach ($categories as $category)
                            @foreach ($category->subcategories as $subcategory)
                                <option value="{{ $subcategory->name }}" data-category-id="{{ $category->id }}" @selected(old('subcategory') === $subcategory->name)>{{ $subcategory->name }}</option>
                            @endforeach
                        @endforeach
                    </select>
                </label>
            ...
            <label>Ceritakan kejadian <span>*</span>
                <textarea name="description" rows="6" minlength="20" required placeholder="Tuliskan apa yang terjadi, kapan, dan di mana. Ceritakan fakta yang Anda lihat atau alami.">{{ old('description') }}</textarea>
...
function updateIdentityFields() {
    ...
    if (mode === 'ANONYMOUS') {
        reporterNameAsterisk.style.display = 'none';
        reporterNameInput.removeAttribute('required');
        reporterNameHelp.textContent = 'Identitas Anda akan disembunyikan sepenuhnya (Anonim).';
    } else {
        reporterNameAsterisk.style.display = 'inline';
        reporterNameInput.setAttribute('required', 'required');
        reporterNameHelp.textContent = 'Wajib diisi untuk mode Rahasia atau Terbuka.';
    }
}

...
Route::prefix('v1')->group(function () {
    Route::post('/auth/login', function () {
        ...
        return response()->json(['token' => $plainToken, 'user' => ['name' => $user->name, 'role' => $user->role, 'school_id' => $user->school_id]]);
    ...
    Route::middleware('api.token')->group(function () {
        ...
        Route::get('/cases', function (\Illuminate\Http\Request $request) {
            ...
            $cases = DB::table('cases')->where('school_id', $user->school_id)->orderByDesc('updated_at')->get();
            return response()->json(['data' => $cases]);
        });
        Route::get('/cases/{caseNumber}', function (\Illuminate\Http\Request $request, string $caseNumber) {
            $user = $request->attributes->get('api_user');
            $case = DB::table('cases')->where('school_id', $user->school_id)->where('case_number', $caseNumber)->firstOrFail();
